...
func add_factory(factory : ProductionNode) -> void:
	if factory == null || productors.has(factory) : return
	...
	if productors.size() + 1 > max_size:
		...
		get_tree().create_tween().tween_property(factory_to_remove,"scale",Vector2.ZERO,0.2).finished.connect(func()->void:factory_to_remove.queue_free())
	
	factory.enable = false
	productors.append(factory)
	get_tree().create_tween().tween_property(factory,"scale",Vector2.ONE*1.3,0.3)
